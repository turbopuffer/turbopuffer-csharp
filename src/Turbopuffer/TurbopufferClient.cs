...
/// <inheritdoc/>
public sealed class TurbopufferClient : ITurbopufferClient
{
    ...
    {
        get { return this._options.Region; }
        init { this._options.Region = value; }
    }

    /// <inheritdoc/>
    public string? DefaultNamespace
    {
        get { return this._options.DefaultNamespace; }
    ...
    /// <inheritdoc/>
    public ITurbopufferClientWithRawResponse WithRawResponse
    {
        get { return _withRawResponse.Value; }
    }

    /// <inheritdoc/>
    public ITurbopufferClient WithOptions(Func<ClientOptions, ClientOptions> modifier)
    {
    ...
    /// <inheritdoc/>
    public async Task<ClientNamespacesPage> Namespaces(
        ClientNamespacesParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.Namespaces1(parameters, cancellationToken)
...
/// <inheritdoc/>
public sealed class TurbopufferClientWithRawResponse : ITurbopufferClientWithRawResponse
{
#if NET
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<ClientNamespacesPage>> Namespaces1(
        ClientNamespacesParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        HttpRequest<ClientNamespacesParams> request = new()
        {
        ...
        return new(
            ...
            async (token) =>
            {
                var page = await response
                    .Deserialize<ClientNamespacesPageResponse>(token)
                    .ConfigureAwait(false);
                if (this.ResponseValidation)
                {
                    page.Validate();
                }
                return new ClientNamespacesPage(this, parameters, page);
            }
        );
    }
        ...
        where T : ParamsBase
    {
        ...
        while (true)
        {
            ...
            if (response != null && (++retries > maxRetries || !ShouldRetry(response)))
            {
                ...
                try
                {
                    throw TurbopufferExceptionFactory.CreateApiException(
                        response.StatusCode,
    ...
    static TimeSpan ComputeRetryBackoff(int retries, HttpResponse? response)
    {
        ...
        var backoffSeconds = Math.Min(0.25 * Math.Pow(2.0, retries - 1), 8.0);
    ...
    {
        return e is IOException || e is TurbopufferIOException;
    }
